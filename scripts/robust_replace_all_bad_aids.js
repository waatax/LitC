...
// Use precise regex replacement matching entry block from `'pid': {` to `}`
let replacedTotal = 0;

for (const [pid, aid] of Object.entries(newAidsMap)) {
  const pKey = pid.replace(/-/g, '\\-');
  const entryRegex = new RegExp(`['"]${pKey}['"]\\s*:\\s*\\{[\\s\\S]*?\\n\\s*\\}`, 'g');
  if (entryRegex.test(readingAidCode)) {
    const replacement = `'${pid}': {\n    translation: ${JSON.stringify(aid.translation)},\n    analysis: ${JSON.stringify(aid.analysis)}\n  }`;
    readingAidCode = readingAidCode.replace(entryRegex, replacement);
    replacedTotal++;
  }
}

fs.writeFileSync('./src/data/readingAid.ts', readingAidCode, 'utf8');
console.log(`Robust replacement completed: Updated ${replacedTotal} passages across all 7 works in readingAid.ts!`);
