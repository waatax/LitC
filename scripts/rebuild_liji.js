...
async function rebuildLiji() {
  const data = loadAllData();
  
  console.log("Fetching Liji chapters from CText...");
  const html = await download('https://ctext.org/liji/zh');
  const regex = new RegExp(`href="liji/([^"]+)"[^>]*>([^<]+)</a>`, 'g');
  const chaptersSpec = [];
  let match;
  ...
  for (const { slug, title } of validSpecs) {
    const url = `https://ctext.org/liji/${slug}`;
    try {
  ...
  console.log("Removing old Liji data...");
  const workId = 'li-ji';
  ...
  for (const [volumeIndex, volume] of downloaded.entries()) {
    ...
    for (const [passageIndex, canonicalText] of volume.texts.entries()) {
      ...
      data.passages.push({
        ...
        canonicalText,
        sentenceIds,
        sourceRefs: [{ label: '經文底本', edition: '中國哲學書電子化計劃《禮記》', url: volume.url }]
      });
    }
  ...
  console.log(`Saving Liji: ${totalChars} chars.`);
  ...
  console.log("Successfully rebuilt Liji!");
}

rebuildLiji().catch(console.error);
