...
import { chapters } from '../src/data/works.ts';
import { passagesPart2 } from '../src/data/sentence_chunks/passages_part2.ts';
import { sentencesPart8 } from '../src/data/sentence_chunks/part8.ts';

async function main() {
  let currentWorks = works.filter(w => w.id !== 'cai-gen-tan');
  currentWorks.push(newWork);

  let currentChapters = chapters.filter(c => c.workId !== 'cai-gen-tan');
...
\`;
  ...
  let currentPassages = passagesPart2.filter(p => !p.id.startsWith('cai-gen-tan_'));
...
\`;
  ...
  let currentSentences = sentencesPart8.filter(s => !s.id.startsWith('cai-gen-tan_'));
...
\`;
  ...
  console.log('Successfully rebuilt Cai Gen Tan into 5 chapters using tsx!');
}
