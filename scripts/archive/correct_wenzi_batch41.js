...
const pairs = {};
for (let target = 14; target <= 31; target += 1) {
  pairs[`wenzi_ch-4_p-${target}`] = `wenzi_ch-3_p-${target + 14}`;
}
for (let target = 1; target <= 20; target += 1) {
  pairs[`wenzi_ch-5_p-${target}`] = `wenzi_ch-3_p-${target + 45}`;
}
...
for (const [passageId, parallelPassageId] of Object.entries(pairs)) {
  ...
  const chapterSource = passageId.includes('ch-4') ? 'https://ctext.org/wenzi/fu-yan/zh' : 'https://ctext.org/wenzi/dao-de/zh';
  reviewData.reviews.push({
    passageId,
    canonicalText: 'pending',
    translation: 'verified',
    analysis: 'verified',
    sources: [chapterSource, 'https://ctext.org/wenzi/jiu-shou/zh', 'https://ctext.org/dao-de-jing/zh', 'https://www.xuanxuecenter.com/files/wen_zi_tong_xuan_zhen_jing.pdf'],
    parallelPassageId,
