...
// Map parsedRows to chapters
// Based on parsing: 
// 2-3 are preface (skip for passage generation or put as preface?) 
// We will follow LitC structure. 
// Rows: 
// 4: '修身' (chapter title), 5-34: passages
// 35: '應酬' (chapter title), 36-81: passages
// 82: '評議' (chapter title), 83-128: passages
// 129: '閑適' (chapter title), 130-371: passages

const chaptersData = [
  { id: 'cai-gen-tan_ch-1', title: '修身', rows: parsedRows.filter(r => r.id >= 5 && r.id <= 34) },
  { id: 'cai-gen-tan_ch-2', title: '應酬', rows: parsedRows.filter(r => r.id >= 36 && r.id <= 81) },
  { id: 'cai-gen-tan_ch-3', title: '評議', rows: parsedRows.filter(r => r.id >= 83 && r.id <= 128) },
  { id: 'cai-gen-tan_ch-4', title: '閑適', rows: parsedRows.filter(r => r.id >= 130 && r.id <= 371) }
];
...
// 2. Helper to extract and write files safely
...
function extractFile(filepath, varName) {
  ...
  const match = content.match(new RegExp(`export const ${varName}(?:\\s*:\\s*[A-Za-z\\[\\]]+)?\\s*=\\s*JSON\\.parse\\('([^]+?)'\\)(?:\\s*as\\s+[A-Za-z\\[\\]]+)?`));
...
export const works = JSON.parse('${jsString(currentWorks)}')
export const chapters = JSON.parse('${jsString(currentChapters)}')
...
console.log('Successfully repaired Cai Gen Tan data!');
