...
async function rebuildShijing() {
  const data = loadAllData();
  
  console.log("Fetching Shijing chapters from CText...");
  const html = await download('https://ctext.org/book-of-poetry/zh');
  const regex = new RegExp(`href="book-of-poetry/([^"]+)"[^>]*>([^<]+)</a>`, 'g');
  const chaptersSpec = [];
  let match;
  ...
  for (const { slug, title } of validSpecs) {
    const url = `https://ctext.org/book-of-poetry/${slug}`;
    try {
  ...
  console.log("Removing old Shijing data...");
  const workId = 'shi-jing';
  ...
  for (const [volumeIndex, volume] of downloaded.entries()) {
    ...
    for (const [passageIndex, canonicalText] of volume.texts.entries()) {
      ...
      data.passages.push({
        ...
        canonicalText,
        sentenceIds,
        sourceRefs: [{ label: '經文底本', edition: '中國哲學書電子化計劃《詩經》', url: volume.url }]
      });
    }
  ...
  console.log(`Saving Shijing: ${totalChars} chars.`);
  ...
  console.log("Successfully rebuilt Shijing!");
}

rebuildShijing().catch(console.error);
