...
> **典籍內容維護規範**：凡涉及原文、白話、解析、來源或覆核狀態，必須遵循 [`docs/EDITORIAL_WORKFLOW.md`](docs/EDITORIAL_WORKFLOW.md)。應用建置成功不等於內容完成；全庫完成以嚴格編校閘門為準。
...
| MVP 範圍 | 道德經優先（81段） | 48 章全收但控制功能範圍（P0/P1/P2） | **採用「48 章全收、功能分階段」**，但**保留道德經作為第一個完整驗證垂直切片（vertical slice）** | 見第 6 章說明 |
...
> 句讀數是以標點初步切分的**上限估計**（尚未區分獨立可背誦句 vs. 附屬子句），正式內容工程階段仍須人工覆核出「Sentence 卡片」的實際數量，預期會少於 966（因為部分子句需合併才有完整語義）。這是 ETL 驗證階段（第 7 章）要處理的工作，不在本規劃階段虛報精確度。

**產品意義**：全部 108 章、約 10,600 字、約 960 句候選卡片，是一個**單人可在 2–3 週內完成人工校對＋分塊**的規模，不需要眾包或自動化 NLP 分句，這直接支持「MVP 全收 108 章」的可行性（呼應第 2 章的裁決）。
...
第 2 章已裁決 MVP 收錄全部 48 章（而非 v2 建議的「先只做道德經 81 段」），理由：
